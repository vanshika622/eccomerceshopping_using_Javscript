...
let getuserobject=JSON.parse(localStorage.getItem("userdetails"))
    // console.log(getuserobject)
    // let user= users.slice();
    let flag=0;
    if(getuserobject){
    for(var i=0;i<users.length;i++) {
        if(users[i].username==getuserobject.username && users[i].password==getuserobject.password){
            flag=1;
            break;
        }
